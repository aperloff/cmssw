...
FactorizedJetCorrectionUncertaintyCalculator::FactorizedJetCorrectionUncertaintyCalculator(const JetCorrectionUncertaintyCollection& fParameters)
{
  ...
  for(unsigned i=0; i<keys.size(); i++) {
    ...
    //mCorrectors is a vector of SimpleJetCorrectionUncertainty
    mCorrectors.push_back(fParameters[keys[i]].getSimpleJetCorrectionUncertainty());
...
{
  std::vector<float> subUnc = getSubUncertainties(iValues);
  float sum = 0;
  float unc_tot = 0;

  for (unsigned int i=0; i<subUnc.size();i++){
    sum += pow(subUnc[i],2);
  }
  unc_tot = sqrt(sum);
  return unc_tot;
...
std::vector<float> FactorizedJetCorrectionUncertaintyCalculator::getSubUncertainties( FactorizedJetCorrectionUncertaintyCalculator::VariableValues& iValues) const
{
  ...
  std::vector<float> subUnc;
  if (mCorrectors.size()<1) std::cout<<"mCorrectors size is 0"<<std::endl;
  for(unsigned int i=0;i<mCorrectors.size();i++) {
    vx = fillVector(mBinTypes[i],iValues);
    vy = fillVector(mParTypes[i],iValues);

    double unc = mCorrectors[i]->uncertainty(vx,vy[0],true);

    subUnc.push_back(unc);
  }
  return subUnc;
  iValues.reset();
}
