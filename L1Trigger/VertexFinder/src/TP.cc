...
namespace l1tVertexFinder {
  ...
  TP::TP() : settings_(nullptr), inTimeBx_(false), physicsCollision_(false), use_(false), useForEff_(false),
             useForAlgEff_(false), useForVertexReco_(false), nLayersWithStubs_(0) {}
  ...
  void TP::fillUseForVertexReco() {
    ...
    if (useForVertexReco_) {
      useForVertexReco_ = (countLayers() >= settings_->genMinStubLayers() and
                           countLayers(true) >= 2);
    }
  }
